...
const validateEmail = (user: { id: number; name: string }): Effect.Effect<{ id: number; name: string; email: string }, UserError> => {
...
const fetchUser = (id: number): Effect.Effect<{ id: number; name: string }, HttpError> =>
  Effect.gen(function* (_) {
    ...
    if (!response.ok) {
      return yield* _(Effect.fail({ _tag: 'NetworkError' as const, message: `HTTP ${response.status}` }))
    }
...
// 使用例
const getUser = (id: number) => 
  apiClient.get<{ id: number; name: string }>(`/api/users/${id}`)

const program = pipe(
  getUser(1),
  Effect.tap((user) => Effect.log(`User: ${user.name}`)),
  Effect.catchTag('NotFoundError', () => 
    Effect.succeed({ id: 0, name: 'Guest' })
  )
)
