...
import { validateUserCreate, validateUserUpdate, type UserCreate, type UserUpdate } from '../schemas/user';
 ...
 * ユーザー取得サービス
 */
export const getUserById = async (id: string): Promise<Result<{ id: string; email: string; name: string | null }, AppError>> => {
  try {
 ...
 * ユーザー更新サービス
 */
export const updateUser = async (id: string, data: unknown): Promise<Result<{ id: string }, AppError>> => {
  // バリデーション
 ...
 * 投稿取得サービス
 */
export const getPostById = async (id: string): Promise<Result<{
  id: string;
  title: string;
  content: string | null;
  published: boolean;
