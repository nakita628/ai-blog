...
const handled = program.pipe(
  Effect.catchAll((error) => Effect.succeed(`Recovered from ${error._tag}: ${error.message}`)),
)
...
const fullyHandled = program.pipe(
  Effect.catchTags({
    HttpError: (error) => Effect.succeed(`HTTP error: ${error.status}`),
...
const withFallback = fetchFromPrimaryApi.pipe(Effect.orElse(() => fetchFromFallbackApi))
...
// 指数バックオフでリトライ
const withExponentialRetry = Effect.retry(fetchData, Schedule.exponential('100 millis'))
```
...
```typescript
// 指数バックオフだが、最大5回まで
const limited = Schedule.intersect(Schedule.exponential('10 millis'), Schedule.recurs(5))
```
...
const fast = Schedule.union(Schedule.exponential('100 millis'), Schedule.spaced('1 second'))
```
...
const staged = Schedule.andThen(Schedule.recurs(5), Schedule.spaced('1 second'))
```
...
```typescript
const policy = Schedule.recurs(3).pipe(Schedule.addDelay(() => '500 millis'))

const program = Effect.retryOrElse(
  fetchData,
  policy,
  // すべてのリトライが失敗した場合のフォールバック
  (error, _) => Effect.succeed({ data: 'default', source: 'fallback' }),
)
```
...
const apiRequest = (
  url: string,
): Effect.Effect<unknown, NetworkError | ApiError | RateLimitError> =>
  Effect.gen(function* () {
...
const robustApiCall = (url: string) =>
  apiRequest(url).pipe(
    ...
    Effect.catchTag('RateLimitError', (err) =>
      Effect.sleep(`${err.retryAfter} millis`).pipe(Effect.andThen(() => apiRequest(url))),
    ),
    // タイムアウト設定
...
const main = robustApiCall('https://api.example.com/data').pipe(
  Effect.catchTags({
    NetworkError: (err) => Effect.succeed({ error: `Network failed: ${err.cause}` }),
    ApiError: (err) => Effect.succeed({ error: `API error ${err.status}: ${err.body}` }),
    RateLimitError: (err) =>
      Effect.succeed({ error: `Rate limited: retry after ${err.retryAfter}ms` }),
  }),
  Effect.catchAllDefect((defect) => Effect.succeed({ error: `Unexpected: ${defect}` })),
)
